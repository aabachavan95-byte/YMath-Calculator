...
const subtopics = JSON.parse(fs.readFileSync('recovered_subtopics.json', 'utf-8'));
...
const mainTopics = [
  ...
  { key: 'ratio_proportion', name: 'प्रमाण आणि समानुपात', englishName: 'Ratio & Proportion', icon: 'RatioIcon', keywords: ['ratio', 'prop', 'simplify', 'combine', 'partnership', 'share', 'mixture', 'coin', 'alligation'] },
...
console.log('Unmatched:', others.length);
if (others.length > 0) {
    console.log(others.map(o => o.key).slice(0, 20));
}
